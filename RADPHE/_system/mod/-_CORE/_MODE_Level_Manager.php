            ...
            if (!in_array($_INTIN['KERNEL']['EVENTlevelFILE'],get_included_files())) {
               ...
               //     $_INTIN['KERNEL']['Tracking']['Blocks']['AllTime']=array_merge_recursive(get_defined_vars(),array('GLOBALS'=>'NO','_INTIN'=>'No'));
                ...
                if (!empty($_INTIN['Dump']))
                    if (memory_get_usage(TRUE)<50000000) $_INTIN['KERNEL']['Tracking']['Memory'][$_INTIN['KERNEL']['EVENT']][$_INTIN['KERNEL']['EVENTlevelFILE']]['GlObAlS']=@array_diff_assoc(array_merge_recursive(get_defined_vars(),array('_INTIN'=>array('MOD'=>'NOPE', 'KERNEL'=>array('Tracking'=>array('No'))))),$_INTIN['KERNEL']['Tracking']['Memory']['LastTime']);
                ...
                if (memory_get_usage(TRUE)<10000000) $_INTIN['KERNEL']['Tracking']['Memory']['LastTime']=array_merge_recursive(get_defined_vars(),array('GLOBALS'=>'','_INTIN'=>NULL));
