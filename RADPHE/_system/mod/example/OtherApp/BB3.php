...
if (
	...
	@$_INTIN['MOD']['CMS']['Blocks']['SupplementryContent'] .=  '<h1 class="text-dark">BB3 Works - In Output Buffer</h1><h2 class="text-dark">Post is passthrough! It WILL WORK ALL THE WAY!</h2><h3 class="text-dark">This is becasue it is assumed to be already in output uncaptured by a block.</h3>';
	return;
}

if (
		(1)&&
		(false !== strpos(dirname($_SERVER["PHP_SELF"]),'bb3'))&&
		(!empty($_INTIN['MOD']['CMS']['Blocks']['Request']))
) {
	CMS_Skinner::$Page['LayoutFile']='WP';
	if ((1)&&((false !== stripos(dirname($_SERVER["PHP_SELF"]),'adm')))) {
		CMS_Blocks::CancelOutput();
		@$_INTIN['MOD']['CMS']['Blocks']['SupplementryContent'] .=  '<h1 class="text-dark">BB3 Works - In block</h1><h2 class="text-dark">Post is passthrough! It WILL WORK ALL THE WAY!</h2><h3 class="text-dark">This is becasue it was captured by a block and hooked in.</h3>';
...
if ((1)&&(false !== strpos(dirname($_SERVER["PHP_SELF"]),'BB3'))) {
	CMS_Skinner::$Page['LayoutFile']='WP';
	@$_INTIN['MOD']['CMS']['Blocks']['SupplementryContent'] .= '<h1 class="text-dark">BB3 Works - by proxy</h1><h2 class="text-dark">Post is currently dropped! It WILL WORK up until interaction!</h2><h3 class="text-dark">This is becasue it a block captured by proxy and post passthrough is not implemented... yet.</h3><pre class="text-dark">You should try the local word press link... its either a manual install or a MOVE or COPY of the one alisously installed in a integrated directory sub site where .htaccess dosent have all.
...
this file needs to be tested outside of contect of running in a BB3 request for its proxy function.
